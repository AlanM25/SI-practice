...
# im = cv2.bilateralFilter(im, 5, 21, 7)

# th, im = cv2.threshold(im,220,255, cv2.THRESH_BINARY);
...
for keyPoint in keypoints:
    ...
    if (s>35):
        ...
        ObjetosPosFiltrado += 1
        imX = cv2.circle(imX, (int(x), int(y)), int(s), (0, 0, 255), 2)
    elif (s>=10 and s<25):
        ObjetosPosFiltrado += 1
        imX = cv2.circle(imX, (int(x), int(y)), int(s), (255, 0, 0), 2)
    else:
        print("Discarded!!")
